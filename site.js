...
function render(initialReport) {
  ...
  $("#competitor-rows").innerHTML = report.competitors
    ...
    .map(
      (c) =>
        `<div class="competitor-row ${c.self ? "self" : ""}" style="--share:${c.share.toFixed(1)}%"><span>${c.self ? '<span class="acme-mark">a</span>' : logo(c.name.toLowerCase())}${c.name}${c.self ? " <small>you</small>" : ""}</span><b>${c.share.toFixed(1)}%</b></div>`,
    )
    .join("");
  renderQuestions();
...
$("#motion-toggle").addEventListener("click", () => {
  ...
  $("#motion-toggle").setAttribute("aria-pressed", paused);
