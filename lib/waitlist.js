...
const sources = ['header', 'hero', 'waitlist', 'closing', 'footer', 'direct'];
...
export function createWaitlist({ store, secret, now = () => Date.now() }) {
  ...
  async function signup(body, ip) {
    ...
    if (await store.create(key, record)) return { ok: true, token: tokenFor(id, body.requestId) };
  ...
  async function profile(body, ip) {
    ...
    const values = { website: normalized, role: body.role || '', goal: body.goal || '' };
